	...
	FOR rec IN
		...
		WHERE t.typname = 'topology.ValidateTopology_ReturnType'
		  AND NOT (
				...
				c.relkind = 'c' AND
				c.relname in ( 'topology.ValidateTopology_ReturnType','topology.TopoGeometry','topology.TopoElement','topology.TopoElementArray','topology.GetFaceEdges_ReturnType' )
			)
	LOOP
		RAISE EXCEPTION
		  'Column "%" of table "%"."%" '
