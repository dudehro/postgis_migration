...
DROP FUNCTION IF EXISTS _st_clip (	rast raster, nband integer[],	geom geometry,	nodataval double precision[] , crop boolean );
DROP FUNCTION IF EXISTS st_clip (	rast raster, nband integer[],	geom geometry,	nodataval double precision[] , crop boolean );
DROP FUNCTION IF EXISTS st_clip (	rast raster, nband integer,	geom geometry,	nodataval double precision, crop boolean );
DROP FUNCTION IF EXISTS st_clip (	rast raster, nband integer,	geom geometry,	crop boolean);
...
DROP FUNCTION IF EXISTS st_makeemptycoverage (tilewidth int, tileheight int, width int, height int, upperleftx float8, upperlefty float8, scalex float8, scaley float8, skewx float8, skewy float8, srid int4 );
