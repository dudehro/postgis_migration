...
function dump_database(){
	...
	pg_dumpall -U kvwmap -d kvwmapsp --globals-only -f roles_tablespaces.dump
	...
	do
		for element in "${dump_tables[@]}"; do
			if [[ $element == "$DB" ]]; then
				echo "Dump DB ${DB}"
				pg_dump -U kvwmap -d kvwmapsp --create --exclude-table='shp_export_*' --format=custom -f schema_data.${DB}.dump "${DB}"
				break
			fi
		done
